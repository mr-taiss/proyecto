...
(function () {
  ...
  function login() {
    ...
    if (!user || password !== user.password) { if (message) { message.textContent = "Usuario o contraseña incorrectos."; message.style.color = "#b44b4b"; } return; }
  ...
  function ownerCanEdit(id) {
    ...
    if (!object || object.owner !== user.name) { alert("Solo el estudiante que registró este objeto puede modificarlo."); return false; }
    return true;
  }
  ...
  function updateUserBadge() {
    ...
    badge.innerHTML = `<strong>${user.name}</strong> <button onclick="logoutSISGOP()" style="margin-left:8px;border:0;background:transparent;color:#b44b4b;cursor:pointer">Salir</button>`;
  }

  window.enterSystem = function () { if (currentUser()) { window.openHome(); updateUserBadge(); } else showLogin(); };
    ...
    if (registerForm && !registerForm.dataset.ownerGuard) {
      ...
      registerForm.addEventListener("submit", () => {
        setTimeout(() => {
          ...
          if (unowned.length) { unowned[unowned.length - 1].owner = user.name; localStorage.setItem(OBJECTS_KEY, JSON.stringify(data)); }
        }, 0);
      });
    }
    ["editObject", "deleteObject", "markFound", "markRecovered"].forEach(name => {
      const original = window[name]; const marker = "__sisgopWrapped";
      if (typeof original === "function" && !original[marker]) { const wrapped = function (id) { if (ownerCanEdit(id)) return original(id); }; wrapped[marker] = true; window[name] = wrapped; }
    });
